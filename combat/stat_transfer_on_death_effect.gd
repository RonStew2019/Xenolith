...
## Transfers the host reactor's stats to a designated transfer_target's reactor when
...
## are added to the transfer_target's reactor (integrity clamped to the new max).
##
## Permanent duration (-1) — lives until the host dies or the effect is
## manually removed.  Zero heat — purely reactive, no tick cost.
##
## Stackable: multiple instances with different transfer_targets can coexist
...
func _init(
	p_transfer_target: Node = null,
	p_source: Node = null,
	p_heat: float = 0.0,
	p_duration: int = -1,
) -> void:
	super._init("StatTransferOnDeath", p_heat, p_duration, p_source, false, true)
	transfer_target = p_transfer_target
...
## transfer_target's reactor.  If the direct [member transfer_target] is
## invalid or dead, walks up the [code]clone_parent[/code] ancestry chain
## to find the nearest living ancestor — ensuring grandchildren with dead
## parents still return stats to grandparents (or further up the chain).
...
## Resolve the transfer target.  Returns the direct [member transfer_target]
## when it is still valid and alive, otherwise walks up the
## [code]clone_parent[/code] chain from the host mech to find the nearest
## living ancestor.  Returns [code]null[/code] when every ancestor in the
## chain is dead or freed.
func _resolve_target() -> Node:
	# Fast path: direct transfer_target is alive — no traversal needed.
	if is_instance_valid(transfer_target):
		var tr := _find_reactor(transfer_target)
